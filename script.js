document.addEventListener("DOMContentLoaded", () => {
    ...
    const chipIcon = document.querySelector(".chip-icon");
    const chipText = document.querySelector(".chip-text");
    const loginForm = document.getElementById("login_form");
    const signinMode = document.getElementById("signin_mode");
    const signupMode = document.getElementById("signup_mode");
    ...
    // Local accounts (data saved in the Windows itself)
    const ACCOUNTS_KEY = "win12_accounts";
    ...
    try {
        ...
        const found = loadAccounts().find((a) => a.email === sessionEmail) || null;
        currentAccount = found;
        if (currentAccount) currentUserName = currentAccount.name;
    } catch (err) {
        currentAccount = null;
    }
    ...
    [loginEmail, passwordInput, signupName, signupEmail, signupPassword].forEach((field) => {
    ...
    avatarInput.addEventListener("change", (e) => {
        ...
        reader.onload = () => {
            draftPicture = reader.result;
            userIcon.src = draftPicture;
            chipIcon.src = draftPicture;
        };
    ...
    function setupLockScreen() {
        ...
        if (currentAccount) {
            ...
            loginEmail.value = currentAccount.email;
            passwordInput.value = "";
            ...
            setLoginMode(accounts.length === 0 ? "signup" : "signin");
            setTimeout(() => {
                const first = loginMode === "signin" ? loginEmail : signupName;
